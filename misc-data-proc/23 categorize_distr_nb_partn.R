...
TIME_PERIODS <- "Pre-Pandemic"

df_results_ls <- list(pre_pand = NULL)

for(cur_time_prd in TIME_PERIODS){
  print(paste(cur_time_prd, "========================"))
  ...
  data_pmf_tmp <- filter(data_pmf, time_prd == cur_time_prd)
  ...
  df_results_ls[[which(cur_time_prd == TIME_PERIODS)]] <- bind_rows(df_results)
}

# Output ----
# verify outputs
{
  df_tmp_pre <- df_results_ls$pre_pand
  
  # check mean rates
  print(
    sprintf("Mean rate for Vancouver:      pre-pandemic -- %s",
            # round(sum(df_tmp_post$mean_rate * df_tmp_post$prop), 1),
            round(sum(df_tmp_pre$mean_rate * df_tmp_pre$prop), 1))
  )
  
  # check minimum and max rates 
  print(
    sprintf("Rate range for Vancouver:     pre-pandemic -- [%s-%s]",
            # round(min(df_tmp_post$mean_rate), 1), round(max(df_tmp_post$mean_rate), 1),
...
df_results_ls$pre_pand$prop; round(df_results_ls$pre_pand$prop, 4)

# df_results_ls$post_main$prop <- round(df_results_ls$post_main$prop, 5)
df_results_ls$pre_pand$prop <- round(df_results_ls$pre_pand$prop, 5)

write_csv(df_results_ls$pre_pand, "./misc-data-proc/outputs-cm/engage_van_actv_grps_nb_partn_p6m.csv")
